...
API/Bot/Admin → Operation(PENDING) → gates_stream
                                   ...
                                   Transaction + AccountMove (двойная запись)
                                         ↓
                                   callbacks_stream → CallbackMicroservice → webhook
```
...
- **Microservices** (`microservices/`) — NATS-консьюмеры: fiscal (транзакции, двойная запись), callbacks (доставка вебхуков с retry)

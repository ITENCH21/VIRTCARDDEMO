...
## Деплой лендинга (landing/index.html)

### Git-репозиторий

- **Локальный путь**: `/Users/ivan/Desktop/01_Архив_проекты/Projects_from_Mac/Виртуальные карты/virt_card_processing/`
- **Remote**: `origin` → `git@github-itench21:ITENCH21/VIRTCARDDEMO.git`
- **Ветка**: `main`

### Сервер

- **SSH**: `ssh virtcardpay-root-claude` (алиас в `~/.ssh/config`, хост `46.173.26.75`, user `root`, ключ `~/.ssh/claude_virtcardpay_root`)
- **Git-клон на сервере**: `/root/virtcardpay` (remote: `ITENCH21/VIRTCARDDEMO.git`)
- **Docker-монтирование**: контейнер `nginx` монтирует `/root/virt_card_processing/landing` → `/landing` внутри контейнера
- **ВАЖНО**: Docker nginx читает файлы из `/root/virt_card_processing/landing/`, а НЕ из `/root/virtcardpay/landing/`. После `git pull` нужно копировать!

### Процесс деплоя (3 шага)

```bash
# 1. Локально: commit + push
cd /Users/ivan/Desktop/01_Архив_проекты/Projects_from_Mac/Виртуальные\ карты/virt_card_processing
git add landing/index.html
git commit -m "описание изменений"
git push origin main

# 2. На сервере: pull + копирование
ssh virtcardpay-root-claude "cd /root/virtcardpay && git pull origin main && cp landing/index.html /root/virt_card_processing/landing/index.html"

# 3. Проверка: убедиться что новый код отдаётся
ssh virtcardpay-root-claude "curl -s https://virtcardpay.ru/ | grep -o 'какой-то-уникальный-текст'"
```

### Nginx-конфиг сервера

- Хостовый nginx (`/etc/nginx/sites-enabled/virtcardpay.ru`): SSL-терминация + proxy_pass → `127.0.0.1:8080`
- Docker nginx (контейнер `nginx`, порт 8080): отдаёт лендинг из `/landing/index.html` с `Cache-Control: no-cache`
- На сервере также есть `/root/virt_card_processing` — это **другой** git-репозиторий (`selezGit/virt_card_processing.git`), не путать!

### Другой проект на этом сервере

- `/root/virt_card_processing` — основной backend-репозиторий (remote: `selezGit/virt_card_processing.git`), Docker-сервисы
- `/root/virtcardpay` — демо-репозиторий для лендинга (remote: `ITENCH21/VIRTCARDDEMO.git`)
